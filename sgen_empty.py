...
def make_chart(alpha0,delta0,N):
	...
	stars=SkyCoord(15*alphaS*u.degree, deltaS*u.degree)
	Ms=SkyCoord(alphaM*u.degree, deltaM*u.degree)

	center = SkyCoord(alpha0*15*u.degree, delta0*u.degree)
...
rem = np.hstack([glob.glob('./M*.tex'), glob.glob('./*.log'), glob.glob('./*.aux'), glob.glob('./*.eps'), glob.glob('./*.dvi'), glob.glob('./*.pp3'), glob.glob('./*.dat')])
for f in rem:
	os.remove(f)
